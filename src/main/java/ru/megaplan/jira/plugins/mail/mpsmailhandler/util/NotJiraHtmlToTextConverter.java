 ...
 * comments.
 */
public class NotJiraHtmlToTextConverter
{
    private static final Category log = Category.getInstance(NotJiraHtmlToTextConverter.class);
    ...
    private class HTMLCallbackHandler extends HTMLEditorKit.ParserCallback {
        ...
        public void handleStartTag(HTML.Tag tag, javax.swing.text.MutableAttributeSet set, int position) {
            try
            {
                ...
                if (inBody && !inQuote && started && tag.equals(HTML.Tag.P))
                {
                    out.write(NEWLINE + NEWLINE);
                }
                else if (inBody && !inQuote &&  started && tag.equals(HTML.Tag.DIV)) {
                    out.write(NEWLINE);
                }
                else if (inBody && !inQuote && started && tag.equals(HTML.Tag.BR)) {
                    out.write(NEWLINE);
                }
                else if (inBody && !inQuote &&  started && tag.equals(HTML.Tag.OL) || tag.equals(HTML.Tag.UL))
                {
                    inList = true;
                    out.write(NEWLINE + NEWLINE);
                ...
                else if (inBody && !inQuote && started && inList && tag.equals(HTML.Tag.LI))
                {
                    out.write(NEWLINE);
                    if(listCount != -1)
                    {
                ...
                else if (inBody && !inQuote &&  started && tag.equals(HTML.Tag.TABLE))
                {
                    out.write(NEWLINE);
                }
                else if (inBody && !inQuote &&  started && tag.equals(HTML.Tag.TR))
                {
                    out.write(NEWLINE);
                    firstTD = true;
                }
                else if (inBody && !inQuote && started && tag.equals(HTML.Tag.TD) || tag.equals(HTML.Tag.TH))
                {
                    if(!firstTD)
                    {
                        out.write(TAB);
                ...
                else if (inBody && !inQuote && started && tag.equals(HTML.Tag.PRE))
                {
                    out.write(NEWLINE);
                }
                else if (inBody && !inQuote && started && tag.equals(HTML.Tag.IMG))
                {
                    // Check if the img has a src attribute
                    handleLink((String)set.getAttribute(HTML.Attribute.SRC));
                }
                else if (inBody && !inQuote && started && tag.equals(HTML.Tag.A))
                {
                    // Check if the img has a src attribute
                    handleLink((String)set.getAttribute(HTML.Attribute.HREF));
                }
                else if (inBody && !inQuote && started && tag.equals(HTML.Tag.HR))
                {
                    out.write(NEWLINE + DASH_LINE);
                }
                else if (inBody && !inQuote && started && tag.equals(HTML.Tag.H1) || tag.equals(HTML.Tag.H2) || tag.equals(HTML.Tag.H3) || tag.equals(HTML.Tag.H4) || tag.equals(HTML.Tag.H5) || tag.equals(HTML.Tag.H6))
                {
                    out.write(NEWLINE);
                }
                else if (inBody && !inQuote && started && tag.equals(HTML.Tag.BLOCKQUOTE)) {
                    inQuote = true;
                }
